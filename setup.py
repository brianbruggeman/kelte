...
def get_package_metadata(top_path=None):
    ...
    metadata.setdefault(
        "tests_require", requirements.get("tests") or requirements.get("test") or []
    )
    ...
    license = get_license() or "Copyright {year} - all rights reserved".format(
        year=year
    )
...
def get_setup_commands():
    ...
    commands = {"clean": CleanCommand, "cli": InstallCommandCompletionCommand}
    return commands
...
def parse_requirements(path):
    ...
    for requirement in req.parse_requirements(path, session="somesession"):
        ...
        spec = (
            str(requirement.req.specifier)
            if len(str(requirement.req.specifier))
            else ""
        )
